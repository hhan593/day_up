...
[以 W3C 规范提供的示意图为例](https://www.w3.org/TR/SVG2/pservers.html#RadialGradients)。上图展示了当 `fr` 是 `r` 的 `50%` 时几何属性的定义。小圆圈标记了最外圆的中心点 `(cx, cy)` ，而十字架标记了最内圆的中心点 `(fx,fy)` 。虚线显示了两个渐变向量。向量连接最内圆和最外圆上的对应点。最外圆外部的区域使用最后一个停止颜色进行绘制，而最内圆内部的区域使用第一个停止颜色进行绘制。
