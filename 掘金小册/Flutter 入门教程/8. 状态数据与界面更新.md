...
| 标题 |  |
| --- | --- |
| ![9c01dd336825f9d53fb4be9c009d241.jpg](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9ec0c580395548159ffd68f4502db283~tplv-k3u1fbpfcp-watermark.image?) | ![a2632258a542b4a881dfe5c2252f1b2.jpg](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d64a1756c94b4ec5aa7080e3a9286ca7~tplv-k3u1fbpfcp-watermark.image?) |
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/cb460c39605240f58a680ac58c2f6a8e~tplv-k3u1fbpfcp-watermark.image?)

也就是在猜数字过程中，要隐藏数字的展示；取除上方的提示字，效果如下：


![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/058492bab8f14c77821691fe15b66438~tplv-k3u1fbpfcp-watermark.image?)

代码实现也比较简单，和之前一样，通过 `_guessing` 值，决定组件构造的内容。从这里可以看出，一个状态数据，可以控制界面中很多部件的表现形式。

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5e993b7c009140b5a9011d109519f66d~tplv-k3u1fbpfcp-watermark.image?)
...
![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a3a68d4e996748dfaf6bd58a06dfc873~tplv-k3u1fbpfcp-watermark.image?)
...
![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fcb96b5a0e96408bbd2bea68b68390fe~tplv-k3u1fbpfcp-watermark.image?)

在 `GuessAppBar` 类中，并没有猜数字过程中的相关数据，在这里校验大小并不是很合适。这时就可以通过回调，将事件触发的任务移交给自己的使用者。因为函数本身也可以视为一个对象，如下所示，将函数作为属性成员，通过构造函数进行赋值：


![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1b3418c3c98b4fc28b88d0ba5fced119~tplv-k3u1fbpfcp-watermark.image?)

这样在构造 `GuessAppBar` 组件时，就可以将回调事件交由 `_GuessPageState` 处理，而这里有我们维护的状态数据。处理如下：


![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/dee3fdcbf7284c6ab7978b718818fb85~tplv-k3u1fbpfcp-watermark.image?)
...
![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b0f85ae9fb6f4724873c313dfb452883~tplv-k3u1fbpfcp-watermark.image?)
