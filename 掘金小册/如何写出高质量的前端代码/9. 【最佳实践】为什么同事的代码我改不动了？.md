...
![vue-core.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4707c17499414089b25eda035eb05293~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=336&h=472&s=19130&e=png&b=ffffff)
...
export default {
    ...
    computed:{
        vipCount(){
            return this.userList.filter(user => user.type === 'vip').length;
        },
        activeVipsCount(){
            return this.userList.filter(user => user.type === 'vip' 
                && dayjs(user.loginTime).isAfter(dayjs().subtract(30, 'day'))).length;
        },
        activeUsersCount(){
            return this.userList.filter(user => dayjs(user.loginTime).isAfter(dayjs().subtract(30, 'day'))).length;
        }
    },
    mounted(){
        this.getUserData();
...
再比如某个第三方库有一些bug，然而也没有可替换的，有的同学就会采用手动修改node_modules下的文件的方式去解决，这就是不正统的解决方案，比较合理的方式应该是通知库的作者修改或者将自己修改后的包发布到企业内部仓库，项目中引用修改后的库。
...
![01-持续产生价值.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/669ff3138cbf495582307e4ed23add82~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=1135&h=998&s=73544&e=png&b=ffffff)
...
export default {
    ...
    computed: {
        ...
        //先把活跃用户列表筛选出来
        activeUsers(){
            return this.userList.filter(user => dayjs(user.loginTime).isAfter(dayjs().subtract(30, 'day')))
        },
        //从活跃用户中查找vip用户数量
        activeVipsCount(){
