...
我们都知道 `cubic-bezier()` 函数定义的方式是 `cubic-bezier(P1x, P1y, P2x, P2y)` 。假设你初设的 `cubic-bezier()` 函数是 `cubic-bezier(0.45, 0.25, 0.6, 0.95)` ，那么它的反转函数则是 `cubic-bezier(0.4, 0.05, 0.55, 0.75)` 。这种反转会将缓动效果从缓入变为缓出，反之亦然。实质上，它在水平方向上翻转了曲线。
