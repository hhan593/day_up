...
乍一看，动画填充模式中的“向前”模式（`forwards`）总是使用 `100%` 关键帧，“向后”模式（`backwards`）总是使用 `0%` 关键帧，但实际上，情况并非总是如此。因为关键帧的 `0%` 和 `100%` 会受到 CSS 的 `animation-direction` 和 `animation-iteration-count` 属性值的影响，动画的第一个关键帧实际上可能不是 `0%` ，而最后一个关键帧可能不是 `100%` 。
