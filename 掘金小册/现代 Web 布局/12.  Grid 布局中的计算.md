通过前面课程的学习，我们可以使用 `grid-template-rows` 和 `grid-template-columns` 设置显式网格轨道数量和尺寸，也可以使用 `grid-auto-rows` 和 `grid-auto-columns` 来设置隐式网格轨道尺寸。虽然它们服务的对象有所差异，但是使用方法几乎是相似的，只有个别函数不能用于 `grid-auto-rows` 和 `grid-auto-columns` 属性上，比如 `repeat()` 函数。
...
![Frame 255.jpg](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/32ce73765aec4c8ea3efa039138ba8d0~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=1295&h=938&s=484756&e=jpg&b=545573)
...
这里要提醒大家的是，**虽然本节课程中的示例大都以** **`grid-template-columns`** **属性为例，但不代表这些计算只存在于该属性上，相关的理论与细节也同样存在于** **`grid-template-rows`** **、`grid-auto-rows`** **和** **`grid-auto-columns`，只不过部分 CSS 的函数不可以运用于** **`grid-auto-rows`** **和** **`grid-auto-columns`** **属性上，比如** **`repeat()`** **函数** 。