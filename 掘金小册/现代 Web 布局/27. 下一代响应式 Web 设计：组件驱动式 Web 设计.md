...
// 获取屏幕宽度和高度的像素值 
var screenWidth = window.screen.width * devicePixelRatio; var screenHeight = window.screen.height * devicePixelRatio; 

// 输出设备像素比例和屏幕分辨率 
console.log("设备像素比例：" + devicePixelRatio); 
console.log("屏幕宽度（像素）：" + screenWidth); 
console.log("屏幕高度（像素）：" + screenHeight);
```
