...
![Frame 127.jpg](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d63283bcd3ce4f62b4077458a839e24f~tplv-k3u1fbpfcp-watermark.image?)
...
![Frame 128.jpg](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f1cfc600f7704952a3ff0a84bdae7b5e~tplv-k3u1fbpfcp-watermark.image?)
...
这两个示例告诉我们，当 Flex 项目显式设置了 `flex:1` 和具体的 `width` 值时，

- 如果浏览器计算出来的 `flex-basis` 大于 Flex 项目设置的 `width` 属性值，那么浏览器会将 `flex-basis` 的计算值视为 Flex 项目的最终宽度 (请查看 `flex: 1` 和 `width: 160px` 的示例)
- 如果浏览器计算出来的 `flex-basis` 小于 Flex 项目设置的 `width` 属性值，那么浏览器会将 Flex 项目的最小内容尺寸（`min-content`）作为该 Flex 项目的宽度（请查看 `flex: 1` 和 `width: 260px` 的示例）
