...
但 VW 也有很大的缺陷，比如给页面设置一个最大宽度，让页面水平居中，它会打破整个页面的布局。因为整个页面的元素尺寸大小都是以 `vw` 单位来计算的。如果你在容器上显式设置一个最大值，使容器自身水平居中，但由于内部元素尺寸是根据视窗大小来设定的，并不会随着外部容器尺寸变化而相应调小，如此一来就会造成页面布局的混乱：
...
注意，示例中 `max-height` 并没有设置 `100%`，而是取了一个较大的固定值 `1000px`，主要是因为`max-height` 取百分比，会致使用例失效（如果其父容器未显式设置 `height` 值，`max-height` 取百分比值会无效）。
...
回过头来想，这种像素缩放技术和 [Bootstrap 团队提出的 RFS 技术](https://github.com/twbs/rfs)是非常相似的，只不过 RFS 技术更专注于字号随着视窗大小调整。而我们今天探讨的技术更像是百分百缩放的技术。这种技术最大的亮点就是**像素理想值视为纯数值，并将其乘以当前视窗宽度与设计稿理想宽度比率** 。
