...
// script start -> async1 start -> async2 -> promise1 -> async1 end -> promise2 -> requestAnimationFrame -> setTimeout

> 微任务和宏任务
> 涉及 `requestAnimationFrame` 的执行时机
> 阅读:
> [字节佬讲解](https://zhuanlan.zhihu.com/p/142742003) > [简书一文](https://www.jianshu.com/p/ba5828330aec)
...
> 知识点：事件循环（主要是 setTimeout 10 ms 在前面，100 ms 在后面）
...
> 考察作用域：y 没有定义，所以会挂载在全局中，z 没赋值所以是 undefined，x 在函数内访问不到
