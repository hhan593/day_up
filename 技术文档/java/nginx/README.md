...
> 衔接：`springboot/README.md`（后端服务）、`microservices/README.md#六api-网关`（网关对比）、`kubernetes/README.md#六ingress`（Ingress 底层即 Nginx）、`network/README.md`（HTTP/HTTPS）、`linux/README.md`（部署与排查）。
...
- 架构：`master` 进程（管理）+ 多个 `worker` 进程（处理请求，= CPU 核数），单 worker 单线程基于 `epoll`（见 `network/README.md#七io-模型`）。
- 性能：轻松支撑数万并发连接，内存占用低。
...
- 透传 `X-Forwarded-*` 头，后端 `Spring Boot` 才能拿到真实 IP（见 `springboot/README.md`）。
...
- 与注册中心：静态配置；动态可用 `Nacos`/`Consul` + `nginx-conf` 自动生成（见 `nacos/README.md`）。
...
- **K8s Ingress**：`kubernetes/README.md#六ingress` 中 ingress-nginx 控制器底层就是 Nginx，把 Ingress 规则翻译成 Nginx 配置。
- **网关对比**：Nginx 偏通用流量层；Spring Cloud Gateway 偏业务网关（鉴权/限流/灰度），二者可分层（Nginx 外层 + Gateway 内层），见 `microservices/README.md#六api-网关`。
...
- 502 多为后端挂掉；504 多为后端慢或阻塞（结合 `linux/README.md` 排查后端）。
