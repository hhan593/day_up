...
- Kafka / RabbitMQ / RocketMQ：解耦、削峰、最终一致（见 `java-middle/24-消息队列与微服务.md`）。
...
- 常见：Spring Cloud Gateway（见 `java-middle/22-SpringSecurity与OAuth2与JWT.md`）、Kong、APISIX。
- 典型路由：
...
- 结合 K8s 探针（见 `kubernetes/README.md#八探针与自愈`）实现自愈。
...
- 详情见 `java-middle/22-SpringSecurity与OAuth2与JWT.md`。
