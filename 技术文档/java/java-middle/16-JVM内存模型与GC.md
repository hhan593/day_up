...
- **先分清两个「内存模型」**：本篇标题里的「内存模型」指 **JVM 运行时内存区域**（堆/元空间/栈/程序计数器）；并发规范中的 **JMM（Java Memory Model，JSR-133）** 讲的是可见性与重排，在 `34-并发底层原理JMM与锁.md`。面试问「JMM」时必须先确认问的是哪个。
- 对象存活判定（GC Roots / 可达性分析）、四种引用、类加载与双亲委派、`jstat`/`jstack`/Arthas 与线上 OOM 排查案例 → `37-JVM调优与故障排查.md`
- 并发锁机制的 API 用法见 `15-并发进阶.md`，底层 Mark Word 与锁状态演化见 `34-并发底层原理JMM与锁.md`；虚拟线程调度与载体线程见 `10-虚拟线程.md`。
- 对比 Go：Go 的 GC 也是并发三色标记、低延迟，但 Go 无分代（1.19 前）；Java ZGC 停顿更可控。
- Spring 应用 OOM/Full GC 频繁时，本篇是排错起点、`37` 是操作手册 → `13-SpringBoot.md`
