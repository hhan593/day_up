...
| `Arrays.toString / deepEquals / sort` | 对象数组 `equals` 只比元素，嵌套必须 `deepEquals`；基础类型 sort 为双轴快排（见 `java_base/09-进阶数组.md`） |
...
- `java_base/02-Java基础概念.md`、`java_base/03-运算符.md`：字面量与运算符的基础用法；本篇补齐其底层机制（常量池、拆箱、数值提升、`+` 的 JEP 280 实现）。
