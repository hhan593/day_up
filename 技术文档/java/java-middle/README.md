...
1. `java_base/00-09`（零基础：语法、数组、方法、面向对象入门）
...
8. `18` JDBC → `19` JPA → `20` MyBatis → `21` 持久层进阶 → **`39` Spring 事务与传播机制**；SQL 侧配套 `../mysql/`（事务与锁的 DB 层论证在 `mysql/09`、`mysql/16`）
...
## 目录状态：整个 `技术文档/java` 有两层

**第一层：编号正文（系统深挖）**

| 目录 | 篇数 | 定位 |
|---|---|---|
| `java_base/` | 10（00-09） | 零基础入门 |
| `java-middle/` | 40 正文 + 本 README = 41 | 语言核心 → 企业栈 → 云原生 → 底层深挖（01-40 连续编号） |
| `mysql/` | 23（00-22） | 从 SQL 到主从分库分表 |

**第二层：`../<主题>/README.md` 知识总纲（13 篇，速查与冲刺）**

`../jvm`、`../concurrency`、`../springboot`、`../redis`、`../mq`、`../distributed`、`../microservices`、`../nacos`、`../kubernetes`、`../docker`、`../nginx`、`../network`、`../linux`（每篇 100-350 行，统一带「常见面试考点」收尾）。

`技术文档/java` 合计 **87 篇 md**。

### 两层怎么配合读

| 主题 | 总纲（速查） | 深挖正文 |
|---|---|---|
| JVM | `../jvm` | `16`（结构与收集器）+ `37`（引用/类加载/工具/案例） |
| 并发 | `../concurrency` | `15`（API）+ `34`（JMM/AQS/锁）+ `35`（线程池/协作）+ `36`（并发容器） |
| Spring | `../springboot` | `13`/`14` + `38`（Web 层）+ `39`（事务）+ `40`（Bean 生命周期）+ `31`（反射与代理） |
| Redis | `../redis` | `23`（Spring Data Redis 用法） |
| 消息队列 | `../mq` | `24`（Spring Kafka/AMQP）+ `27`（Kafka Streams）+ `32`（零拷贝根因） |
| 微服务/注册配置 | `../microservices`、`../nacos` | `24`（Spring Cloud 代码层）+ `30`（可观测性） |
| 容器编排 | `../docker`、`../kubernetes` | `29`（Spring Boot 侧落地） |
| 分布式理论 | `../distributed` | `39` 第十二节（事务取舍）+ `23`（分布式锁） |
| 计算机基础 | `../network`、`../linux` | `32`（IO 模型与多路复用） |
...
| 主题 | 现状 |
|---|---|
| Netty 专题 | `32` 覆盖 NIO 三大件与 Reactor 模型、`../network` 覆盖 IO 模型，但 `EventLoop`/`ChannelPipeline`/`ByteBuf`/黏包拆包/编解码器无独立篇 |
| Maven / Gradle 工程与依赖管理 | 仅 `../springboot` 有 `pom.xml` 片段；scope、依赖仲裁 nearest-wins、BOM、私服、多模块无专篇 |
| Elasticsearch | 全目录仅出现在本索引的待补充清单里，0 实质内容 |
| 分布式事务落地 | `../distributed` 与 `39` 第十二节给了方案与选型，Seata AT/TCC、本地消息表、RocketMQ 事务消息无落地篇 |
| 中间件运维件 | Canal/binlog 订阅（`../redis` 提了一句）、xxl-job、Apollo（已有 `../nacos` 可替代）、Sentinel 细节 |
| 压测与性能工程 | JMeter/wrk/gatling、火焰图基准测试无覆盖 |
| 架构方法论 | DDD 分层（`../microservices` 有服务拆分原则）、SOLID 设计原则（`25` 只有 GoF 模式清单） |
