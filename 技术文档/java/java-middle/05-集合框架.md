...
集合常作为 Stream 数据源（见 `07-Lambda与函数式编程.md`）：
```java
...
- **本篇是用法层**（怎么选、怎么写）；底层结构、put 流程推演、树化阈值、`ConcurrentHashMap` 演进、fail-fast 机制见 `36-集合底层源码剖析.md`。
- `String`/包装类做 key 的 `equals`/`hashCode` 契约与缓存陷阱 → `33-字符串与常用API.md`
- 阻塞队列作为线程池工作队列 → `35-线程池与线程协作.md`
- 集合 + Stream 是函数式处理数据的核心 → `07-Lambda与函数式编程.md`
