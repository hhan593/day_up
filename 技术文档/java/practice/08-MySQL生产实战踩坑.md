...
> 基础见 `../mysql/`（00-22）；优化手段见 `../mysql/12-SQL优化.md`。
...
- binlog 一次写一大坨 → **从库单线程回放这一大坨，主从延迟飙到分钟级**（`../mysql/20-主从复制.md`）；
- 锁持有数分钟 → 沿途业务全阻塞。
...
`../mysql/12-SQL优化.md` 讲了手段，这里是**线上真实体验**：
...
| 用 UUID varchar 当主键 | 随机写入页分裂 + 二级索引膨胀（`../mysql/12-SQL优化.md`） |
...
| 备份没验证过 | "有备份"≠"能恢复"：定期演练恢复（`../mysql/18-MySQL管理.md`） |
...
```text
1. 慢查询日志（阈值 1s）每日巡检 → top N（../mysql/19-日志.md）
2. EXPLAIN 分析：type/rows/Extra（../mysql/11-索引.md）
...
- 索引与 EXPLAIN：`../mysql/11-索引.md`
- 锁与死锁：`../mysql/16-锁.md`
- 日志与慢查询：`../mysql/19-日志.md`
