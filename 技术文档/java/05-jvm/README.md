...
> 衔接：`02-java-middle/16-JVM内存模型与GC.md`（结构与收集器）、`02-java-middle/37-JVM调优与故障排查.md`（GC Roots/四种引用/类加载与双亲委派/诊断命令与 8 个线上案例，**总纲第六、七、八节的深挖全在那**）、`02-java-middle/34-并发底层原理JMM与锁.md`（另一个 JMM：并发规范）、`04-concurrency/README.md`（内存模型差异）、`13-linux/README.md`（排查命令）。
...
- 容器场景：`kubectl exec` + 上述命令，或 Sidecar 采集（见 `15-kubernetes/README.md`）。
