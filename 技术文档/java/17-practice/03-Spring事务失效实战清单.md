...
> 传播行为/隔离级别理论见 `../02-java-middle/39-Spring事务与传播机制.md`、`../08-springboot/README.md#七事务管理`。
...
- MySQL 表引擎是 **MyISAM**（`../03-mysql/10-存储引擎.md`）：SQL 全部照常执行，回滚无效。
- 多数据源场景：`@Transactional` 只管**主数据源**的事务管理器，另一个库不受控。
- **修复**：InnoDB；多库用指定 `transactionManager`，或放弃跨库强一致改最终一致（`../10-distributed/README.md#六分布式事务`）。
...
危害：连接占用久 → 池耗尽；undo log 巨大；锁持有久 → 死锁/阻塞（`../03-mysql/16-锁.md`）；主从延迟。
...
- 传播行为/隔离级别全解：`../02-java-middle/39-Spring事务与传播机制.md`
- MySQL 事务与锁：`../03-mysql/09-事务.md`、`../03-mysql/16-锁.md`
