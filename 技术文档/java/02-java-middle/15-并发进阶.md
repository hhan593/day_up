...
> 补充：JEP 444 虚拟线程（已详述于 `10-虚拟线程.md`）
> 说明：Oracle 并发教程为 JS 渲染页，下列机制基于 Java SE 标准 API 与官方教程结构整理。

Java 并发从底层锁到高层异步编排，工具链完整。本文覆盖传统并发核心，与 `10-虚拟线程.md` 互补。
...
- 虚拟线程（`10-虚拟线程.md`）下，**同步阻塞代码即高吞吐**，多数场景无需手动 `CompletableFuture` 编排。
...
- **本篇是 API 用法层**；`volatile` 为什么不够、`synchronized` 锁在对象头哪个字段、`ReentrantLock` 的 AQS 队列怎么排 → `34-并发底层原理JMM与锁.md`
- 线程池七参数与执行流程推演、拒绝策略、优雅关闭、线程数怎么定、`wait/notify` 与三大同步工具、`ThreadLocal` 泄漏 → `35-线程池与线程协作.md`
- `ConcurrentHashMap` 的桶锁与 `size` 计数实现 → `36-集合底层源码剖析.md`
- 线程 dump 与死锁定位 → `37-JVM调优与故障排查.md`
- 虚拟线程是现代并发首选 → `10-虚拟线程.md`
- 对比 JS/TS（Node 单线程事件循环）：Java 是真多线程 + 锁，复杂但有并行能力；
  对比 Go：goroutine ≈ 虚拟线程，channel ≈ `BlockingQueue`。
- Spring 中 `@Async` 注解底层即线程池 + `CompletableFuture` → `13-SpringBoot.md`
